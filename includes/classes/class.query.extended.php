...
class db {
  ...
  public $result;
  public $totalRow;
  public $DiplayErrorsEndUser = true;
  public $dbErrorMsg;
  private $error = array();
  private $host;
  private $driver;
  ...
  public function connect($config = array()) {
    ...
    try {
      ...
      $this->tbl_pre = $config['table_prefix'];
   ...
   * $db->result();
   */
  public function query($queryString, $method = PDO::FETCH_BOTH) {
    try {
      //query
  ...
  public function select($qryArray) {
    ...
    $qryStr = 'SELECT SQL_CALC_FOUND_ROWS ' . $fetchFields . ' FROM ' . $qryArray['tbl_name'] . ' ' . ((isset($qryArray['condition']) && $qryArray['condition'] != null) ? $qryArray['condition'] : '');
  ...
  public function insert($tableName, $dataArray = array(), $unique = array()) {
    ...
    if (count($unique) > 0) {
      ...
      $cQryStr = "SELECT " . $unique[0] . " FROM " . $this->tbl_pre . "" . $tableName . " WHERE " . implode('AND ', $condition);
    ...
    if (!$duplicate) {
      $qryStr = 'INSERT INTO ' . $this->tbl_pre . '' . $tableName . ' (' . $rawFieldsStr . ') VALUES(' . $fields_str . ')';
  ...
  public function update($tableName, $dataArray = array(), $where, $unique = array()) {
    ...
    if (!$duplicate && ($where != null || (is_array($where) && count($where) > 0))) {
      ...
      $qryStr = 'UPDATE ' . $this->tbl_pre . '' . $tableName . ' SET ' . $fields_str . $whereCond;
      try {
        //query
        $affectedRow = $this->safe_execution($dataArray, $qryStr);
      } catch (PDOException $e) {
  ...
  public function delete($tableName, $where) {
    ...
    if ($where != null || (is_array($where) && count($where)) > 0) {
      ...
      $qryStr = 'DELETE FROM ' . $this->tbl_pre . '' . $tableName . ' ' . $whereCond;
