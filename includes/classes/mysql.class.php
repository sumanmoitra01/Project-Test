...
class DB {
	 ...
	 */
	var $link_id = 0;
	// An resource of the database link identifier.
	var $query_id = 0;
	// An resource of the query.
	var $record = array();
	// An array of the rows.
	var $valid_charset = '';
	// Valid MySQL character set.
	var $num_rows = 0;
	// The number of count of the rows.
	 ...
	 * @param resource The result of the query to take action.
	 */
	function fetch_object($result = -1) {
		if ($result != -1) {
			$this->query_id = $result;
		}
		if (isset($this->query_id)) {
